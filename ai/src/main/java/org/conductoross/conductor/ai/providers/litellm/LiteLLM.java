...
public class LiteLLM implements AIModel {

    public static final String NAME = "litellm";
    private final LiteLLMConfiguration config;
    private final OpenAICompatChatModel chatModel;

    public LiteLLM(LiteLLMConfiguration config) {
        this(config, AIHttpClients.defaultClient());
    }

    public LiteLLM(LiteLLMConfiguration config, OkHttpClient httpClient) {
        this.config = config;
