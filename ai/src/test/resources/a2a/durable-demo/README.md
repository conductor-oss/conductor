...
```
  ...
  │     └─ CALL_AGENT  ──────────┘     (poll while "preparing")  │  separate process,       │
...
- **Concierge** = the `durable_purchase` Conductor workflow (`durable_purchase.json`): one
  `CALL_AGENT` task that places the order with the remote agent and polls until it's ready.
...
- **Durable store** = SQLite (Conductor's zero-dependency standalone mode). The workflow + the
  `CALL_AGENT` task (with the remote agent's task id) are persisted here; on restart the engine
...
```
▶ Placing an order (the concierge calls the restaurant agent via CALL_AGENT)
...
- This demo doubles as the **full-process crash/restart proof** for A2A durability — a real
  `kill -9` of a persistent server with `CALL_AGENT` resuming — complementing the in-process
