...
| `A2ADurableEngineEndToEndTest` (`test-harness`) | `AGENT` through the **real engine** (decider + `AsyncSystemTaskExecutor` + Redis), proving crash/restart resume | yes (needs Docker for Redis) |
| `A2ARealAgentIntegrationTest` | Discovery + `AGENT` against a **real external agent** | only when `A2A_AGENT_URL` is set |
...
> `AGENT` returned `state=completed`, `text=echo-task: convert 100 USD`.

## Full-server verification (manual)

To exercise the whole engine path (workflow → engine → `AGENT` system task → real agent):
...
4. Confirm the workflow completes and the `AGENT` task output carries the agent's
   `state`, `text`, `artifacts`, `taskId`, and `contextId`. Use the `conductor` CLI/skill to
   start the workflow and inspect the execution.

## Task types

- `AGENT` — send a message to a remote agent; poll (default), stream (`streaming:true`),
...
- `CANCEL_AGENT` — cancel a running remote agent task.
