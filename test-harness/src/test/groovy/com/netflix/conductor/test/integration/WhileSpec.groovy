...
class WhileSpec extends AbstractSpecification {
    ...
    def setup() {
        workflowTestUtil.registerWorkflows('while_integration_test.json',
                'while_multiple_integration_test.json',
                'while_as_subtask_integration_test.json',
                'while_iteration_fix_test.json',
                'while_sub_workflow_integration_test.json',
                'while_five_loop_over_integration_test.json',
                'while_system_tasks.json',
                'while_with_decision_task.json',
                'while_set_variable_fix.json')
    }
    ...
    def "Test workflow with 2 iterations of five tasks"() {
        ...
        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("while_five_loop_over_integration_test", 1, "looptest", workflowInput, null)

        then: "Verify that the workflow has started"
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            status == Workflow.WorkflowStatus.RUNNING
            tasks.size() == 4
            tasks[0].taskType == 'WHILE'
        ...
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            ...
            tasks[11].iteration == 0 // this is outside WHILE
        }
    ...
    def "Test workflow with 2 iterations of 3 system tasks"() {
        ...
        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("while_system_tasks", 1, "looptest", workflowInput, null)
    ...
    def "Test workflow with a single iteration While task"() {
        given: "Number of iterations of the loop is set to 1"
        def workflowInput = new HashMap()
        workflowInput['loop'] = 1

        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("While_Workflow", 1, "looptest", workflowInput, null)
    ...
    def "Test workflow with a single iteration While task with Sub workflow"() {
        given: "Number of iterations of the loop is set to 1"
        def workflowInput = new HashMap()
        workflowInput['loop'] = 1

        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("While_Sub_Workflow", 1, "looptest", workflowInput, null)
    ...
    def "Test workflow with multiple While tasks with multiple iterations"() {
        given: "Number of iterations of the first loop is set to 2 and second loop is set to 1"
        def workflowInput = new HashMap()
        workflowInput['loop'] = 2
        workflowInput['loop2'] = 1

        when: "A workflow with multiple while tasks with multiple iterations is started"
        def workflowInstanceId = startWorkflow("While_Multiple", 1, "looptest", workflowInput, null)
    ...
    def "Test retrying a failed while workflow"() {
    ...
    def "Test auto retrying a failed while workflow"() {
    ...
    def "Test workflow with a iteration While task as subtask of a forkjoin task"() {
        given: "Number of iterations of the loop is set to 1"
        def workflowInput = new HashMap()
        workflowInput['loop'] = 1

        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("While_SubTask", 1, "looptest", workflowInput, null)
        ...
        when: "Polling and completing first task in While"
        ...
        when: "Polling and completing second task in While"
        Tuple polledAndCompletedTask1 = workflowTestUtil.pollAndCompleteTask('integration_task_1', 'integration.test.worker')
    ...
    def "Test workflow with While task contains loop over task that use iteration in script expression"() {
        given: "Number of iterations of the loop is set to 2"
        def workflowInput = new HashMap()
        workflowInput['loop'] = 2

        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("While_Workflow_Iteration_Fix", 1, "looptest", workflowInput, null)
    ...
    def "Test workflow with While task has no iteration"() {
        given: "The loop condition is set to use set variable"
        def workflowInput = new HashMap()
        workflowInput['value'] = 2
        workflowInput['list'] = null

        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("while_Set_variable_fix", 1, "looptest", workflowInput, null)
        ...
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            ...
            tasks[0].taskType == 'WHILE'
            tasks[0].status == Task.Status.COMPLETED
            tasks[0].outputData['iteration'] == 0
        }
    }

    def "Test workflow with While task contains set variable task"() {
        given: "The loop condition is set to use set variable"
        def workflowInput = new HashMap()
        workflowInput['value'] = 2
        workflowInput['list'] = [1]
        ...
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            ...
            tasks[1].status == Task.Status.COMPLETED
            tasks[1].inputData.get("value") == workflowInput['value']
            variables['value'] == workflowInput['value']
        }
    }

    def "Test workflow with While task contains decision task"() {
        given: "The loop condition is set to use set variable"
        def workflowInput = new HashMap()
        ...
        when: "A while workflow is started"
        def workflowInstanceId = startWorkflow("While_with_Decision_task", 1, "looptest", workflowInput, null)
