...
class HierarchicalForkJoinSubworkflowRetrySpec extends AbstractSpecification {
    ...
    def setup() {
        ...
        and: "the subworkflow task in SCHEDULED state is started by issuing a system task call"
    ...
    def "Test retry on the root in a 3-level subworkflow"() {
        ...
        asyncSystemTaskExecutor.execute(subWorkflowTask, polledTaskIds[0])
        def newMidLevelWorkflowId = workflowExecutionService.getExecutionStatus(rootWorkflowId, true).tasks[4].subWorkflowId
    ...
    def "Test retry on the mid-level in a 3-level subworkflow"() {
        ...
        def polledTaskIds = queueDAO.pop(TASK_TYPE_SUB_WORKFLOW, 1, 200)
        asyncSystemTaskExecutor.execute(subWorkflowTask, polledTaskIds[0])
        def newLeafWorkflowId = workflowExecutionService.getExecutionStatus(midLevelWorkflowId, true).tasks[4].subWorkflowId
