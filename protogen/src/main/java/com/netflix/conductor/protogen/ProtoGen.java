...
public class ProtoGen {
    ...
    public static ClassName CUSTOM_MAPPER_CLASS = ClassName.get(GENERATED_MAPPER_PACKAGE, "ProtoMapperBase");
    ...
    public void writeMapper(String root) throws Exception {
        TypeSpec.Builder protoMapper = TypeSpec.classBuilder("ProtoMapper")
                .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                .addAnnotation(AnnotationSpec.builder(Generated.class)
                        .addMember("value", "$S", GENERATOR_NAME).build())
                .superclass(CUSTOM_MAPPER_CLASS);

        for (File file : files) {
            file.getMessage().generateJavaMapper(protoMapper);
        }

        JavaFile javaFile = JavaFile.builder(GENERATED_MAPPER_PACKAGE, protoMapper.build())
                .indent("    ").build();
        Path filename = Paths.get(root, "ProtoMapper.java");
