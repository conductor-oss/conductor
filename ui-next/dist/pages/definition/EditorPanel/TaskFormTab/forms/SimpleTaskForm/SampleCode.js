const r=o=>o?.length?o.map(t=>`@InputParam("${t}") Object ${t}`).join(", "):"",i=({taskDefName:o,inputParamKeys:t})=>`/*
...
public class Main {
    ...
    public static void main(String[] args)
    {
        ...
        ApiClient apiClient = new ApiClient("${window.location.origin}/api","some-key","some-secret");
    ...
    @WorkerTask("${o}")
    public String greet(${r(t)}) {
        ${t&&t?.length>0?`return String.format("Hello ${t?.map(e=>"%s").join(", ")}!", ${t?.map(e=>`convertToString(${e})`).join(", ")});`:'return "Hello";'}
    }
}
`,a=({taskDefName:o,inputParamKeys:t})=>`# To set up the project, install the dependencies, and run the application, follow these steps:
#
...
@worker_task(task_definition_name='${o}')
def greet(${t&&t?.length>0?`${t?.map(e=>`${e}: str`)}`:""}):
    return f'Hello ${t&&t?.length>0?t?.map(e=>`{${e}}`):"there!"}'

api_config = Configuration()

task_handler = TaskHandler(configuration=api_config)
task_handler.start_processes() # starts polling for work
# task_handler.stop_processes() # stops polling for work`,s=({taskDefName:o,inputParamKeys:t})=>`/*
...
func Greet(task *model.Task) (interface{}, error) {
 var greetings strings.Builder
    ...
    ${t.map(e=>`if val, ok := task.InputData["${e}"]; ok {
...
func main() {
	taskRunner.StartWorker("${o}", Greet, 1, time.Millisecond*100)
    taskRunner.WaitWorkers();
}
`,c=({taskDefName:o,inputParamKeys:t})=>`/*
 * To set up the project, install the dependencies, and run the application, follow these steps:
 *
...
public class SimpleWorker: IWorkflowTask
{
    ...
    public SimpleWorker(string taskType = "${o}")
    {
        TaskType = taskType;
        ...
        default)
    {
        return await System.Threading.Tasks.Task.Run(() =>
        {
            ...
            result.OutputData = new Dictionary < string, object >
            {
                {
                    "result",
                    outputString ${t&&t?.length>0?`+= " " + string.Join(" ", [${t.map(e=>`task.InputData["${e}"]`)}])`:""}
                }
            };
            return result;
        });
...
}`,l=({taskDefName:o,accessToken:t,inputParamKeys:e})=>`/*
 * To set up the project, install the dependencies, and run the application, follow these steps:
 *
...
async function test() {
  ...
  const customWorker = {
    taskDefName: "${o}",
    execute: async ({ inputData${e&&e?.length>0?`:{ ${e} }`:""}, taskId }) => {
      return {
        outputData: {
          greeting: \`Hello World ${e?.map(n=>`\${${n}}`)}\`,
        },
        status: "COMPLETED",
      };
    },
...
test();`,u=({taskDefName:o,accessToken:t,inputParamKeys:e})=>`/*
 * To set up the project, install the dependencies, and run the application, follow these steps:
 *
 * 1. Install the Conductor JavaScript SDK:
 *    npm install @io-orkes/conductor-javascript
...
`;export{c as sampleCSharpCode,p as sampleClojureCode,s as sampleGolangCode,i as sampleJavaCode,l as sampleJavaScriptCode,a as samplePythonCode,u as sampleTypeScriptCode};
