...
describe("Agent AdvancedSearch — carrying basic-only filters into SQL format", () => {
  beforeEach(() => vi.clearAllMocks());

  it("searches with the agent name filter from basic search", async () => {
    await renderAdvanced("?asQuery=true&workflowType=TestAgent-Aug");

    expect(lastQuery()).toContain("workflowType IN (TestAgent-Aug)");
  });

  it("carries the execution id, correlation ids and idempotency keys", async () => {
    await renderAdvanced(
      "?asQuery=true&workflowId=abc-123&correlationIds=c1&idempotencyKey=k1",
    );

    expect(lastQuery()).toContain("workflowId='abc-123'");
    expect(lastQuery()).toContain("correlationId IN (c1)");
    expect(lastQuery()).toContain("idempotencyKey IN (k1)");
  });
  ...
  it("shows the carried-over filters in the SQL box", async () => {
    ...
    expect(screen.getByLabelText("Search")).toHaveValue(
      "workflowType IN (TestAgent-Aug)",
    );
  });

  it("does not override a query the user already typed", async () => {
    await renderAdvanced(
      "?asQuery=true&workflowType=TestAgent-Aug&query=workflowId%3D%27typed%27",
    );

    expect(lastQuery()).toContain("workflowId='typed'");
    expect(lastQuery()).not.toContain("TestAgent-Aug");
  });

  it("respects an explicitly cleared query box", async () => {
    await renderAdvanced("?asQuery=true&workflowType=TestAgent-Aug&query=");
