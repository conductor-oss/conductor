...
/** Makes escaped line breaks inside recalled JSON evidence readable in the preview. */
function readablePromptText(message: PromptMessage): string {
  return promptMessageText(message).replaceAll("\\n", "\n");
}

type RecalledMemoryPrompt = {
  context: string;
  payload: unknown;
};

/** Separates the human guidance from the JSON payload generated by OCG recall. */
function recalledMemoryPrompt(text: string): RecalledMemoryPrompt | undefined {
  if (!text.startsWith("# Relevant prior memory")) return undefined;
  const jsonStart = text.indexOf("\n{");
  if (jsonStart < 0) return undefined;

  try {
    return {
      context: text.slice(0, jsonStart).trim(),
  ...
  return (
    <Box sx={{ overflowY: "auto", p: 2.5 }}>
      ...
      {messages.map((message, index) => {
        ...
        const rawText = promptMessageText(message);
        const recall = recalledMemoryPrompt(rawText);
        const text = readablePromptText(message);
        const isLongSystemMessage =
          role === "system" && !recall && text.length > 1200;
        const isExpanded = expanded[index] ?? !isLongSystemMessage;
              ...
              {recall ? (
                <>
                  <Box sx={{ whiteSpace: "pre-wrap" }}>
                    <ContentView value={recall.context} />
                  </Box>
                  <Typography
                    sx={{
                      fontSize: "0.75rem",
                ...
                <>
                  {isInstructions ? (
                    <InstructionView
                      content={displayedText || "(empty message)"}
                    />
                  ) : (
                    <Box sx={{ whiteSpace: "pre-wrap" }}>
