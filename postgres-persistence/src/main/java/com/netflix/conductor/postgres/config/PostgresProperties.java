...
public class PostgresProperties {
    ...
    @Value("${conductor.postgres.connection.pool.size.max:-1}")
    private int connectionPoolMaxSize;

    @Value("${conductor.postgres.connection.pool.idle.min:-1}")
    private int connectionPoolMinIdle;

    @Value("${conductor.postgres.connection.lifetime.max:#{T(java.util.concurrent.TimeUnit).MINUTES.toMillis(30)}}")
    private long connectionMaxLifetime;

    @Value("${conductor.postgres.connection.idle.timeout:#{T(java.util.concurrent.TimeUnit).MINUTES.toMillis(10)}}")
    private long connectionIdleTimeout;

    @Value("${conductor.postgres.connection.timeout:#{T(java.util.concurrent.TimeUnit).MINUTES.toMillis(30)}}")
    private long connectionTimeout;

    @Value("${conductor.postgres.transaction.isolation.level:}")
    private String transactionIsolationLevel;

    /**
     * This is consistent with the current default when building the Hikari Client.
     */
    @Value("${conductor.postgres.autocommit:false}")
