...
public class AgentConfig {
    ...
    @Builder.Default private Strategy strategy = Strategy.HANDOFF;

    /**
     * Multi-agent orchestration strategy — mirrors the Python SDK's {@code Strategy} enum
     * (referenced by name, e.g. {@code strategy=Strategy.PLAN_EXECUTE}, in SDK-facing error
     * messages). Wire format is the lowercase snake_case {@link #toValue()} spelling (e.g. {@code
     * "round_robin"}), matching every persisted {@code metadata.agentDef}/{@code
     * normalizedAgentDef} row and REST payload written before this was an enum — {@link #fromValue}
     * accepts that exact spelling back.
     */
     ...
     * {@link Strategy#PLAN_EXECUTE}. The planner can be a simple agent or a multi-agent (e.g.
     * SEQUENTIAL of explorer + planner). Replaces the old positional {@code agents.get(0)}.
     */
     ...
     * </ul>
     *
     * <p>Only meaningful when {@link #strategy} is {@link Strategy#PLAN_EXECUTE}; the compiler
     * emits HTTP fetch tasks for URL entries inside the planner-route live branch (skipped when
     * {@code static_plan} is set, so the static-plan path stays free of fetch latency).
     */
    ...
    /**
     * Collect worker tool task names from the agent config for domain routing. Worker tools (@tool
     * functions with type "worker" or "cli") are dispatched dynamically via FORK_JOIN_DYNAMIC and
     * must be explicitly added to taskToDomain.
     */
    public void collectWorkerToolNames(Map<String, String> taskToDomain, String domain) {

        if (this.getTools() != null) {
            for (ToolConfig tool : this.getTools()) {
                if (tool.isStateful()) {
        ...
        if (this.getAgents() != null) {
            for (AgentConfig sub : this.getAgents()) {
                sub.collectWorkerToolNames(taskToDomain, domain);
            }
        }
    }
}
