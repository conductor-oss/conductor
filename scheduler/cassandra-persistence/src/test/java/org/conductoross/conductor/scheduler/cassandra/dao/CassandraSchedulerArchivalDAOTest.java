...
public class CassandraSchedulerArchivalDAOTest {
    ...
    private CassandraSchedulerArchivalDAO dao;
    ...
    @Before
    public void setUp() {
        session.execute("DROP TABLE IF EXISTS " + KEYSPACE + ".scheduler_archival_executions");
        session.execute("DROP TABLE IF EXISTS " + KEYSPACE + ".scheduler_archival_by_id");
        CassandraProperties properties = new CassandraProperties();
        properties.setKeyspace(KEYSPACE);
        dao = new CassandraSchedulerArchivalDAO(session, objectMapper, properties);
    }
    ...
    public void testCleanupOldRecords() {
        // Insert 10 records for the same schedule with increasing scheduled_time
        for (int i = 0; i < 10; i++) {
        ...
        // Verify only 3 remain (the most recent ones due to DESC clustering)
        SearchResult<String> result =
