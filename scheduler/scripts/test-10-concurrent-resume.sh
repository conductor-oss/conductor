...
#
# Usage (machine 1, runs setup + fires):
#   ./test-10-concurrent-resume.sh setup http://localhost:8080
#   ./test-10-concurrent-resume.sh fire   http://localhost:8080 <TARGET_EPOCH>
#
# Usage (machine 2, fires only):
#   ./test-10-concurrent-resume.sh fire   http://<other-host>:8080 <TARGET_EPOCH>
...
if [[ "$MODE" == "setup" ]]; then
  ...
  echo "  machine 1: ./test-10-concurrent-resume.sh fire http://localhost:8080 $TARGET_EPOCH"
  echo "  machine 2: ./test-10-concurrent-resume.sh fire http://<other-host>:8080 $TARGET_EPOCH"
  echo "================================================================"
  exit 0
fi
