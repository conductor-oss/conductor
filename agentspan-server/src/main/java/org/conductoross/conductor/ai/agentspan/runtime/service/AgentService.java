...
import org.conductoross.conductor.ai.agentspan.runtime.util.ModelParser;
...
public class AgentService {
    ...
    /**
     * Compile, register workflow + task definitions, and start execution. Supports both native
     * AgentConfig and framework-specific raw configs.
     */
    @SuppressWarnings("unchecked")
    public StartResponse start(StartRequest request) {
        ...
        String cwd = ".";
        if (request.getRawConfig() != null
                && request.getRawConfig().get("cwd") instanceof String rawCwd) {
            cwd = rawCwd;
        }
        input.put("cwd", cwd);
        ...
        // CredentialDisclosureService.record() crashes with NPE on the not-null execution_id
        // column.
        // Use the host's configured ID generator (time-based when embedded in orkes) so the host
        // can
     ...
     *
     * <p>Wraps Conductor's {@code ExecutionService.removeWorkflow} to hard-delete completed
    ...
    /**
     * Resolve the AgentConfig from a StartRequest. If {@code framework} is set, normalize the raw
     * config via the appropriate normalizer. Otherwise, use the native {@code agentConfig} field
     * directly.
     */
    ...
    /**
     * Walk a compiled WorkflowDef tree and return the names of every SIMPLE task. SDKs use this
     * list to know exactly which workers to register.
     */
     ...
     */
    private void collectWorkerToolNames(
            AgentConfig config, Map<String, String> taskToDomain, String domain) {
        if (config == null) return;
        if (config.getTools() != null) {
            for (ToolConfig tool : config.getTools()) {
                if (tool.isStateful()) {
