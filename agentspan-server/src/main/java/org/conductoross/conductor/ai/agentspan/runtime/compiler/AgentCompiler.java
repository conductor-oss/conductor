...
import org.conductoross.conductor.ai.agentspan.runtime.util.ModelParser.ParsedModel;
    ...
    public WorkflowDef compile(AgentConfig config) {
        ...
        if (isFrameworkPassthrough(config)) {
            ...
            if (hasNamedSlots && !isPlanExecute) {
                throw new IllegalArgumentException(
                        "Named slots ``planner=`` and ``fallback=`` are only valid with "
                                ...
                                + config.getName()
                                + "' has strategy='"
                                + (strategy == null ? "(unset → handoff)" : strategy)
            ...
            boolean isMultiAgentStrategy =
                    strategy != null && !strategy.isEmpty() && !"handoff".equals(strategy);
