...
class MultiAgentCompilerTest {
    ...
    void testAgentNamedDoneIsRejected() {
        for (AgentConfig.Strategy strategy :
                List.of(AgentConfig.Strategy.HANDOFF, AgentConfig.Strategy.ROUTER)) {
            AgentConfig config =
                    AgentConfig.builder()
                            .name("team")
                            .model("openai/gpt-4o")
    ...
    @Test
    void testPlanExecuteSubWorkflowForwardsCwdAndMediaWithoutCredentials() {
        // Execution context includes cwd and media, but credentials are delivered through
        // Task.runtimeMetadata and must never be persisted in workflow input.
        AgentConfig planner = simpleSubAgent("planner", "Write a plan");
        AgentConfig harness =
                AgentConfig.builder()
                        .name("forwarding")
        ...
        assertThat(inputs).doesNotContainKey("credentials");
    }
