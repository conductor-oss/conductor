...
public class FailureWorkflowTests {
    ...
    private static TypeReference<List<TaskDef>> TASK_DEF_LIST = new TypeReference<List<TaskDef>>(){};
    ...
    public static void init() throws IOException {
        ...
        //Load metadata
        InputStream resource = FailureWorkflowTests.class.getResourceAsStream("/metadata/workflow_data.json");
        if (resource != null) {
        ...
        if (resource != null) {
            List<TaskDef> taskDefs = objectMapper.readValue(new InputStreamReader(resource), TASK_DEF_LIST);
    ...
    public void testFailureTasksAreNotLost(){
        ...
        for (String workflowId : retried) {
            try {
                await().atMost(15, TimeUnit.SECONDS).pollInterval(10, TimeUnit.MILLISECONDS)
                        .untilAsserted(() -> {
                            Workflow workflow = workflowClient.getWorkflow(workflowId, true);
    ...
    public void testFailureWorkflowInputs() {
        ...
        workflow = taskClient.updateTaskSync(workflowId, workflow.getTasks().get(0).getReferenceTaskName(), TaskResult.Status.COMPLETED, output);
