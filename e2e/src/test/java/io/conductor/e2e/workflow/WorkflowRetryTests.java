...
public class WorkflowRetryTests {
    ...
    public enum InnerFailStatus {
        ...
        InnerFailStatus(TaskResult.Status s) { this.status = s; }
    }

    @ParameterizedTest(name = "innerFailStatus={0}")
    @EnumSource(InnerFailStatus.class)
    @DisplayName("Retry parent with resumeSubworkflowTasks=true retries failed inner task in place across unsuccessful statuses")
    public void testRetryWithResumeFlagInForkPlusSubWorkflowAcrossFailedStatuses(InnerFailStatus failStatus) {
        ...
        registerChildWithSequentialSimpleTasks(failingChildName, List.of(taskARef, taskBRef, taskCRef));
        registerChildWithSequentialSimpleTasks(cancelledChildName, List.of(taskDRef));
        registerParentForkWithTwoSubWorkflowBranches(parentName,
                failingBranchRef, failingChildName, cancelledBranchRef, cancelledChildName);
        ...
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            assertNotNull(findTaskByRef(wf, failingBranchRef).getSubWorkflowId(),
                    "failing branch child must be scheduled");
            assertNotNull(findTaskByRef(wf, cancelledBranchRef).getSubWorkflowId(),
        ...
        String originalCancelledChildId = findTaskByRef(parentSnapshot, cancelledBranchRef).getSubWorkflowId();

        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertNotNull(findActiveTaskByRef(failingChildId, taskARef),
                        "task A must be active before we complete it"));
        completeTaskWithStatus(failingChildId, findActiveTaskByRef(failingChildId, taskARef).getTaskId(),
                TaskResult.Status.COMPLETED);

        await().atMost(10, TimeUnit.SECONDS).untilAsserted(() ->
                assertEquals(Task.Status.COMPLETED, statusOf(failingChildId, taskARef),
        ...
        await().atMost(10, TimeUnit.SECONDS).untilAsserted(() ->
                assertNotNull(findActiveTaskByRef(failingChildId, taskBRef),
                        "task B must become active after A completes"));
        completeTaskWithStatus(failingChildId, findActiveTaskByRef(failingChildId, taskBRef).getTaskId(),
                failStatus.status);

        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            Workflow parent = workflowClient.getWorkflow(parentId, true);
            assertEquals(Workflow.WorkflowStatus.FAILED, parent.getStatus(),
                    "parent should be FAILED after inner task " + failStatus);
            assertEquals(Task.Status.CANCELED, findTaskByRef(parent, cancelledBranchRef).getStatus(),
                    "cancelled sibling SUB_WORKFLOW task should be CANCELED");
            assertNotNull(findTaskByRef(parent, failingBranchRef).getReasonForIncompletion(),
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() ->
                assertEquals(Workflow.WorkflowStatus.RUNNING,
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            assertNull(cancelledBranchNow.getReasonForIncompletion(),
                    "cancelled branch reasonForIncompletion must be cleared");

            Workflow failingChild = workflowClient.getWorkflow(failingChildId, true);
            assertFalse(failingChild.getStatus().isTerminal(),
                    "resumed failing child must be non-terminal, got " + failingChild.getStatus());

            Task aAfterRetry = failingChild.getTasks().stream()
                    .filter(t -> taskARef.equals(t.getReferenceTaskName())
            ...
            assertEquals(originalATaskId, aAfterRetry.getTaskId(),
                    "task A taskId must be UNCHANGED — proves it was not re-executed");
            assertEquals(originalAEndTime, aAfterRetry.getEndTime(),
        ...
        completeTaskWithStatus(failingChildId,
                findActiveTaskByRef(failingChildId, taskBRef).getTaskId(), TaskResult.Status.COMPLETED);

        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertNotNull(findActiveTaskByRef(failingChildId, taskCRef),
                        "task C should be active after B completes"));
        completeTaskWithStatus(failingChildId,
                findActiveTaskByRef(failingChildId, taskCRef).getTaskId(), TaskResult.Status.COMPLETED);

        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertNotNull(findActiveTaskByRef(originalCancelledChildId, taskDRef),
                        "task D should be active in resumed cancelled child"));
        completeTaskWithStatus(originalCancelledChildId,
                findActiveTaskByRef(originalCancelledChildId, taskDRef).getTaskId(), TaskResult.Status.COMPLETED);
    ...
    private void registerParentForkWithTwoSubWorkflowBranches(String parentName,
                                                              String branchARef, String childAName,
                                                              String branchBRef, String childBName) {
    ...
    private Task findTaskByRef(Workflow wf, String ref) {
        return wf.getTasks().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new AssertionError(
                        "task " + ref + " not found in workflow " + wf.getWorkflowId()));
    }
    ...
    private void completeTaskWithStatus(String workflowId, String taskId, TaskResult.Status status) {
        TaskResult tr = new TaskResult();
        tr.setWorkflowInstanceId(workflowId);
        tr.setTaskId(taskId);
        tr.setStatus(status);
        if (status == TaskResult.Status.FAILED || status == TaskResult.Status.FAILED_WITH_TERMINAL_ERROR) {
     ...
     * SIMPLE fails. Retry parent with resumeSubworkflowTasks=true. Asserts:
     ...
     */
    @Test
    @DisplayName("Retry with resumeSubworkflowTasks=true on exact prod shape: in-place reset on cancelled sibling, iter 1 untouched")
    public void testRetryWithResumeFlagSecondIterationFailureInForkInsideDoWhile() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            assertEquals(Task.Status.COMPLETED, inlineAfter.getStatus(), "iter 2 branchB inline must remain COMPLETED");
    ...
    private void completeActiveSimpleRef(String childId, TaskResult.Status status) {
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertNotNull(findActiveTaskByRef(childId, "simple_ref"),
                        "simple_ref must be active in " + childId));
        completeTaskWithStatus(childId, findActiveTaskByRef(childId, "simple_ref").getTaskId(), status);
    }
    ...
    private void awaitWorkflowStatus(String workflowId, Workflow.WorkflowStatus expected) {
        await().atMost(30, TimeUnit.SECONDS).untilAsserted(() ->
                assertEquals(expected, workflowClient.getWorkflow(workflowId, false).getStatus(),
    ...
    private Iter1Snapshot snapshotIter1Child(String childId) {
        ...
        return new Iter1Snapshot(wf.getEndTime(),
    ...
    /**
     * 3-branch FORK inside a single-iteration DO_WHILE. Drive iter 1 so one branch
     * COMPLETES, one FAILS with FAILED_WITH_TERMINAL_ERROR, and one is CANCELED by
     * the fork. Retry WITHOUT resumeSubworkflowTasks: assert the FAILED + CANCELED
     * branches get FRESH subWorkflowIds (taskToBeRescheduled spawn-fresh path),
     * the COMPLETED branch is untouched.
     */
    @Test
    @DisplayName("Retry without resumeSubworkflowTasks: fresh children for unsuccessful SUB_WORKFLOW branches, COMPLETED branch untouched")
    public void testRetryWithoutResumeFlagSpawnsFreshChildrenForUnsuccessfulSubWorkflowSiblings() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            assertEquals(c.childCompleted, currentSubWorkflowId(c.parentId, "sub_workflow_ref_2__1", Task.Status.COMPLETED),
                    "COMPLETED branch must keep its original subWorkflowId");
            String activeFailingChild = activeSubWorkflowId(c.parentId, "sub_workflow_ref__1");
            String activeCancelledChild = activeSubWorkflowId(c.parentId, "sub_workflow_ref_1__1");
            assertNotEquals(c.childFailing, activeFailingChild,
                    "FAILED branch must have a NEW subWorkflowId (no resume flag)");
            assertNotEquals(c.childCancelled, activeCancelledChild,
                    "CANCELED branch must have a NEW subWorkflowId (no resume flag)");
            assertEquals(Workflow.WorkflowStatus.FAILED, workflowClient.getWorkflow(c.childFailing, false).getStatus(),
                    "original failing child must stay FAILED");
            assertTrue(workflowClient.getWorkflow(c.childCancelled, false).getStatus().isTerminal(),
    ...
    /**
     * Same 3-branch setup. Retry WITH resumeSubworkflowTasks=true: assert the FAILED
     * + CANCELED branches keep their original subWorkflowId (in-place restore via
     * walk-up + cancelled-sibling fix), and the COMPLETED branch is untouched.
     */
    @Test
    @DisplayName("Retry with resumeSubworkflowTasks=true: in-place restore of unsuccessful SUB_WORKFLOW branches, COMPLETED branch untouched")
    public void testRetryWithResumeFlagRestoresUnsuccessfulSubWorkflowSiblingsInPlace() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            assertEquals(c.childCompleted, currentSubWorkflowId(c.parentId, "sub_workflow_ref_2__1", Task.Status.COMPLETED),
                    "COMPLETED branch must keep its original subWorkflowId");
            assertEquals(c.childFailing, activeSubWorkflowId(c.parentId, "sub_workflow_ref__1"),
                    "FAILED branch subWorkflowId must be PRESERVED (resume flag)");
            assertEquals(c.childCancelled, activeSubWorkflowId(c.parentId, "sub_workflow_ref_1__1"),
                    "CANCELED branch subWorkflowId must be PRESERVED (resume flag)");
            assertFalse(workflowClient.getWorkflow(c.childFailing, false).getStatus().isTerminal(),
                    "original failing child must be restored to non-terminal");
            assertFalse(workflowClient.getWorkflow(c.childCancelled, false).getStatus().isTerminal(),
                    "original cancelled child must be restored to non-terminal");

            assertInlinePreserved(c.childFailing, c.inlineFailingTaskId, c.inlineFailingEndTime,
                    "failing branch child");
            assertInlinePreserved(c.childCancelled, c.inlineCancelledTaskId, c.inlineCancelledEndTime,
                    "cancelled branch child");
            assertInlinePreserved(c.childCompleted, c.inlineCompletedTaskId, c.inlineCompletedEndTime,
    ...
    private void assertInlinePreserved(String childId, String expectedTaskId, long expectedEndTime, String label) {
        Task inline = onlyTaskByRef(childId, "inline_ref");
        assertEquals(Task.Status.COMPLETED, inline.getStatus(),
                label + " inline_ref must remain COMPLETED after retry");
        assertEquals(expectedTaskId, inline.getTaskId(),
                label + " inline_ref taskId must be UNCHANGED (no re-execution)");
        assertEquals(expectedEndTime, inline.getEndTime(),
    ...
    private static final class Captured {
        ...
        Captured(String p, String f, String c1, String c2,
                 String ifId, long ifEnd, String icId, long icEnd, String icompId, long icompEnd) {
            parentId = p; childFailing = f; childCancelled = c1; childCompleted = c2;
            inlineFailingTaskId = ifId; inlineFailingEndTime = ifEnd;
            inlineCancelledTaskId = icId; inlineCancelledEndTime = icEnd;
            inlineCompletedTaskId = icompId; inlineCompletedEndTime = icompEnd;
        }
    }
    ...
    private Captured setupThreeBranchScenarioAndFail() {
        ...
        Task forkTaskNow = workflowClient.getWorkflow(parentId, true).getTasks().stream()
        ...
        assertEquals(Task.Status.COMPLETED, forkTaskNow.getStatus(),
        ...
        return new Captured(parentId, cB[0], cA[0], cC[0],
                inlineF.getTaskId(), inlineF.getEndTime(),
                inlineCa.getTaskId(), inlineCa.getEndTime(),
                inlineCo.getTaskId(), inlineCo.getEndTime());
    }

    private void registerExactShapeParentAndChildWithThreeBranches(String parentName, String childName) {
        TaskDef simpleDef = new TaskDef("simple");
        simpleDef.setRetryCount(0);
        simpleDef.setOwnerEmail("test@conductor.io");
        metadataClient.registerTaskDefs(List.of(simpleDef));
    ...
    private String currentSubWorkflowId(String parentId, String exactRef, Task.Status expectedStatus) {
        return workflowClient.getWorkflow(parentId, true).getTasks().stream()
                .filter(t -> exactRef.equals(t.getReferenceTaskName()) && t.getStatus() == expectedStatus)
                .findFirst()
                .orElseThrow(() -> new AssertionError("no " + exactRef + " task with status " + expectedStatus))
                .getSubWorkflowId();
    }

    private String activeSubWorkflowId(String parentId, String exactRef) {
        return workflowClient.getWorkflow(parentId, true).getTasks().stream()
                .filter(t -> exactRef.equals(t.getReferenceTaskName()) && !t.getStatus().isTerminal())
                .findFirst()
    ...
    /**
     * FORK_JOIN_DYNAMIC where all branches are SUB_WORKFLOW. One branch will be COMPLETED,
     * one FAILED_WITH_TERMINAL_ERROR, one CANCELED by the fork. Retry WITH resume flag
     * must restore both unsuccessful branches in place (subWorkflowId preserved).
     */
    @Test
    @DisplayName("Retry with resumeSubworkflowTasks=true on FORK_JOIN_DYNAMIC: in-place restore of unsuccessful branches, COMPLETED branch untouched")
    public void testRetryWithResumeFlagInDynamicForkRestoresUnsuccessfulBranchesInPlace() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            assertEquals(c.childCompleted, currentSubWorkflowId(c.parentId, c.completedRef, Task.Status.COMPLETED),
                    "COMPLETED branch must keep its original subWorkflowId");
            assertEquals(c.childFailing, activeSubWorkflowId(c.parentId, c.failingRef),
                    "FAILED branch subWorkflowId must be PRESERVED (resume flag)");
            assertEquals(c.childCancelled, activeSubWorkflowId(c.parentId, c.cancelledRef),
                    "CANCELED branch subWorkflowId must be PRESERVED (resume flag)");
            assertFalse(workflowClient.getWorkflow(c.childFailing, false).getStatus().isTerminal(),
                    "original failing child must be restored to non-terminal");
            assertFalse(workflowClient.getWorkflow(c.childCancelled, false).getStatus().isTerminal(),
    ...
    /**
     * Same shape, retry WITHOUT the resume flag — taskToBeRescheduled nulls the
     * SUB_WORKFLOW siblings' subWorkflowId so each FAILED/CANCELED dynamic branch
     * gets a fresh child.
     */
    @Test
    @DisplayName("Retry without resumeSubworkflowTasks on FORK_JOIN_DYNAMIC: fresh children for unsuccessful branches, COMPLETED branch untouched")
    public void testRetryWithoutResumeFlagInDynamicForkSpawnsFreshChildrenForUnsuccessfulBranches() {
        DynamicCaptured c = setupDynamicForkScenarioAndFail();
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            assertEquals(c.childCompleted, currentSubWorkflowId(c.parentId, c.completedRef, Task.Status.COMPLETED),
                    "COMPLETED branch must keep its original subWorkflowId");
            String activeFailingChild = activeSubWorkflowId(c.parentId, c.failingRef);
            String activeCancelledChild = activeSubWorkflowId(c.parentId, c.cancelledRef);
            assertNotEquals(c.childFailing, activeFailingChild,
                    "FAILED branch must have a NEW subWorkflowId (no resume flag)");
            assertNotEquals(c.childCancelled, activeCancelledChild,
                    "CANCELED branch must have a NEW subWorkflowId (no resume flag)");
    ...
    private static final class DynamicCaptured {
        ...
        final String inlineCompletedTaskId;
        final long inlineCompletedEndTime;
        DynamicCaptured(String p, String fr, String cr, String compr,
                        String f, String c1, String c2,
                        String ifId, long ifEnd, String icId, long icEnd, String icompId, long icompEnd) {
            parentId = p; failingRef = fr; cancelledRef = cr; completedRef = compr;
            childFailing = f; childCancelled = c1; childCompleted = c2;
            inlineFailingTaskId = ifId; inlineFailingEndTime = ifEnd;
            inlineCancelledTaskId = icId; inlineCancelledEndTime = icEnd;
            inlineCompletedTaskId = icompId; inlineCompletedEndTime = icompEnd;
        }
    ...
    private DynamicCaptured setupDynamicForkScenarioAndFail() {
        ...
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertNotNull(findActiveTaskByRef(parentId, "prep_ref"),
        ...
        assertEquals(Task.Status.COMPLETED, dynForkNow.getStatus(),
        ...
        return new DynamicCaptured(parentId, failingRef, cancelledRef, completedRef,
                cF[0], cCa[0], cCo[0],
                inlineF.getTaskId(), inlineF.getEndTime(),
                inlineCa.getTaskId(), inlineCa.getEndTime(),
                inlineCo.getTaskId(), inlineCo.getEndTime());
    ...
    /**
     * Retry on the FAILING child sub-workflow directly (one of the dynamic-fork branches),
     * not on the parent. Walk-up via updateAndPushParents from that child reaches the parent,
     * where the cancelled-sibling fix fires: cancelled branch restored in place
     * (subWorkflowId preserved), COMPLETED branch untouched, failing child's simple_ref
     * retried in place.
     */
    @Test
    @DisplayName("Retry on the FAILING dynamic-fork child sub-workflow directly: walk-up restores cancelled sibling in place, COMPLETED untouched")
    ...
    /**
     * Customer-reported regression after PR #3458: a fork where every branch is a
     * SUB_WORKFLOW (no SIMPLE siblings). When one sub-workflow fails and others get
     * CANCELLED, retry/rerun must resume the cancelled siblings — and to avoid
     * re-executing already-COMPLETED work inside the cancelled children, it must do
     * so by retrying the existing terminated child workflows in-place (preserving
     * subWorkflowId) rather than spawning brand-new ones.
     */
    @Test
    @DisplayName("Retry sub-workflow inside all-SUB_WORKFLOW fork retries each CANCELLED sibling in-place")
    public void testRetrySubWorkflowReschedulesAllSubWorkflowSiblingsInFork() {
        ...
        try {
            ...
            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                ...
                Task failingSub = findActiveTask(wf, failingSubRef, "failing_sub_ref not found");
                Task cancelledSub = findActiveTask(wf, cancelledSubRef, "cancelled_sub_ref not found");
                assertNotNull(failingSub.getSubWorkflowId());
                assertNotNull(cancelledSub.getSubWorkflowId());
                assertFalse(workflowClient.getWorkflow(failingSub.getSubWorkflowId(), true).getTasks().isEmpty());
                assertFalse(workflowClient.getWorkflow(cancelledSub.getSubWorkflowId(), true).getTasks().isEmpty());
            });
            ...
            completeTask(findActiveTask(failingSubWorkflowId, taskInFailingSubRef,
                    "failing inner task not active"), TaskResult.Status.FAILED);

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                Workflow wf = workflowClient.getWorkflow(workflowId, true);
                assertEquals(Workflow.WorkflowStatus.FAILED, wf.getStatus(), "Parent should be FAILED");
            ...
            awaitWorkflowStatus(workflowId, Workflow.WorkflowStatus.RUNNING, "Parent should be RUNNING after retry");

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                Task cancelledSiblingNow = findActiveTask(workflowId, cancelledSubRef,
                        "Expected an active cancelled_sub_ref task after retry");
                assertEquals(originalCancelledSubWorkflowId, cancelledSiblingNow.getSubWorkflowId(),
                        "subWorkflowId must be PRESERVED (retry-in-place), not regenerated");
                Workflow resumedChild = workflowClient.getWorkflow(cancelledSiblingNow.getSubWorkflowId(), true);
                assertFalse(resumedChild.getStatus().isTerminal(), "Resumed child must be non-terminal");
                assertFalse(resumedChild.getTasks().isEmpty(), "Resumed child must still have its tasks");

                Task resumedFailingSub = findActiveTask(workflowId, failingSubRef,
                        "failing_sub_ref must be active after retry");
                assertNull(resumedFailingSub.getReasonForIncompletion(),
                        "failing_sub_ref reasonForIncompletion must be cleared after retry walk-up");
            });

            String resumedChildId = findActiveTask(workflowId, cancelledSubRef,
                    "active cancelled_sub_ref missing").getSubWorkflowId();
            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(resumedChildId, taskInCancelledSubRef,
                            "Inner task in resumed child should be active"));
            completeTask(findActiveTask(resumedChildId, taskInCancelledSubRef,
                    "resumed inner task missing"), TaskResult.Status.COMPLETED);

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(failingSubWorkflowId, taskInFailingSubRef,
                            "Retried task in failing sub-workflow should be active"));
            completeTask(findActiveTask(failingSubWorkflowId, taskInFailingSubRef,
                    "retried failing inner task missing"), TaskResult.Status.COMPLETED);

            awaitWorkflowStatus(workflowId, Workflow.WorkflowStatus.COMPLETED, 20,
    ...
    /**
     * Customer-reported scenario: nested SUB_WORKFLOW forks. When the user retries from the
     * deepest failed task, updateAndPushParents walks up TWO levels. CANCELLED SUB_WORKFLOW
     * siblings at BOTH levels must spawn fresh children. Without the fix, neither level would recover.
     */
    @Test
    @DisplayName("Retry of deeply-nested sub-workflow reschedules CANCELLED SUB_WORKFLOW siblings at every parent level")
    public void testRetryNestedSubWorkflowsReschedulesSiblingsAtEveryLevel() {
        ...
        registerWorkflow(midWfName, List.of(
                forkJoin("inner_fork", List.of(
                        List.of(subWorkflowTask("inner_failing_ref", innerFailingWfName)),
                        List.of(subWorkflowTask("inner_cancelled_ref", innerCancelledWfName)))),
                join("inner_join", List.of("inner_failing_ref", "inner_cancelled_ref"))));

        registerWorkflow(topWfName, List.of(
                forkJoin("top_fork", List.of(
                        List.of(subWorkflowTask("mid_ref", midWfName)),
                        List.of(subWorkflowTask("outer_cancelled_ref", outerCancelledWfName)))),
                join("top_join", List.of("mid_ref", "outer_cancelled_ref"))), 900);
        ...
        try {
            ...
            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                ...
                Task midTask = top.getTasks().stream()
                        .filter(t -> "mid_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("mid_ref not found"));
                Task outerCancelled = top.getTasks().stream()
                        .filter(t -> "outer_cancelled_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("outer_cancelled_ref not found"));
                ...
                Task innerFailing = mid.getTasks().stream()
                        .filter(t -> "inner_failing_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("inner_failing_ref not found"));
                Task innerCancelled = mid.getTasks().stream()
                        .filter(t -> "inner_cancelled_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("inner_cancelled_ref not found"));
                ...
                assertFalse(workflowClient.getWorkflow(innerFailingIdHolder[0], true).getTasks().isEmpty());
            });
            ...
            completeTask(findActiveTask(innerFailingId, innerFailingTaskRef,
                    "inner failing task not active"), TaskResult.Status.FAILED);

            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                Workflow top = workflowClient.getWorkflow(topId, true);
                assertEquals(Workflow.WorkflowStatus.FAILED, top.getStatus(),
                        "Top should be FAILED. Got: " + top.getStatus());
                assertEquals(Task.Status.CANCELED, statusOf(top, "outer_cancelled_ref"),
                        "outer_cancelled_ref should be CANCELED");
                assertNotNull(taskOf(top, "mid_ref").getReasonForIncompletion(),
                ...
                assertEquals(Task.Status.CANCELED, statusOf(mid, "inner_cancelled_ref"),
                        "inner_cancelled_ref should be CANCELED");
                assertNotNull(taskOf(mid, "inner_failing_ref").getReasonForIncompletion(),
            ...
            awaitWorkflowStatus(topId, Workflow.WorkflowStatus.RUNNING, 20, "Top should resume to RUNNING after retry");
            awaitWorkflowStatus(midId, Workflow.WorkflowStatus.RUNNING, 20, "Mid should resume to RUNNING after retry");

            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                Task active = findActiveTask(midId, "inner_cancelled_ref",
                        "Expected an active inner_cancelled_ref after retry");
                assertEquals(originalInnerCancelledId, active.getSubWorkflowId(),
                        "Mid level: subWorkflowId must be PRESERVED (retry-in-place)");
                assertFalse(workflowClient.getWorkflow(active.getSubWorkflowId(), true).getStatus().isTerminal(),
                        "Mid level resumed child must be non-terminal");
                assertNull(findActiveTask(midId, "inner_failing_ref",
            ...
            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                Task active = findActiveTask(topId, "outer_cancelled_ref",
                        "Expected an active outer_cancelled_ref after retry");
                assertEquals(originalOuterCancelledId, active.getSubWorkflowId(),
                        "Top level: subWorkflowId must be PRESERVED (retry-in-place)");
                assertFalse(workflowClient.getWorkflow(active.getSubWorkflowId(), true).getStatus().isTerminal(),
                        "Top level resumed child must be non-terminal");
                assertNull(findActiveTask(topId, "mid_ref",
                                "mid_ref must be active after retry").getReasonForIncompletion(),
                        "top.mid_ref reasonForIncompletion must be cleared after retry walk-up");
            });

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(originalInnerCancelledId, innerCancelledTaskRef,
                            "inner_cancelled inner task must be active"));
            completeTask(findActiveTask(originalInnerCancelledId, innerCancelledTaskRef,
                    "inner_cancelled inner task missing"), TaskResult.Status.COMPLETED);

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(originalOuterCancelledId, outerCancelledTaskRef,
                            "outer_cancelled inner task must be active"));
            completeTask(findActiveTask(originalOuterCancelledId, outerCancelledTaskRef,
                    "outer_cancelled inner task missing"), TaskResult.Status.COMPLETED);

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(innerFailingId, innerFailingTaskRef,
                            "retried inner failing task must be active"));
            completeTask(findActiveTask(innerFailingId, innerFailingTaskRef,
                    "retried inner failing task missing"), TaskResult.Status.COMPLETED);

            awaitWorkflowStatus(topId, Workflow.WorkflowStatus.COMPLETED, 30,
                    "Top workflow should reach COMPLETED end-to-end");

        } finally {
            terminateAll(topWfName, midWfName, innerFailingWfName, innerCancelledWfName, outerCancelledWfName);
        }
    }

    /**
     * Retrying from the leaf failed task walks updateAndPushParents through three
     * parent levels. SIMPLE cancelled siblings at the grandchild and mid levels are
     * resumed by the existing else-branch (reset to SCHEDULED + re-queue). The
     * SUB_WORKFLOW cancelled sibling at the top level is resumed in-place by the new
     * branch (existing child id preserved).
     */
    @Test
    ...
    public void testRetryNestedForkWithSimpleAndSubWorkflowSiblings() {
        ...
        registerWorkflow(grandchildSubWfName, List.of(
                forkJoin("grandchild_fork", List.of(
                        List.of(subWorkflowTask("failing_leaf_ref", failingLeafWfName)),
                        List.of(simpleTask(grandchildCancelledSimpleRef)))),
                join("grandchild_join", List.of("failing_leaf_ref", grandchildCancelledSimpleRef))));

        registerWorkflow(midSubWfName, List.of(
                forkJoin("mid_fork", List.of(
                        List.of(subWorkflowTask("grandchild_ref", grandchildSubWfName)),
        ...
        registerWorkflow(topWfName, List.of(
                forkJoin("top_fork", List.of(
                        ...
                        List.of(subWorkflowTask("top_cancelled_ref", topCancelledSubWfName)))),
                join("top_join", List.of("mid_ref", "top_cancelled_ref"))), 900);
        ...
        try {
            ...
            await().atMost(30, TimeUnit.SECONDS).untilAsserted(() -> {
                ...
                Task midTask = top.getTasks().stream()
                        .filter(t -> "mid_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("mid_ref not found"));
                Task topCancelled = top.getTasks().stream()
                        .filter(t -> "top_cancelled_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("top_cancelled_ref not found"));
                assertNotNull(midTask.getSubWorkflowId());
                ...
                Workflow mid = workflowClient.getWorkflow(midIdHolder[0], true);
                Task grandchild = mid.getTasks().stream()
                        .filter(t -> "grandchild_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("grandchild_ref not found"));
                assertNotNull(grandchild.getSubWorkflowId());
                grandchildIdHolder[0] = grandchild.getSubWorkflowId();

                Workflow grandchildWf = workflowClient.getWorkflow(grandchildIdHolder[0], true);
                Task leaf = grandchildWf.getTasks().stream()
                        .filter(t -> "failing_leaf_ref".equals(t.getReferenceTaskName()))
                        .findFirst().orElseThrow(() -> new AssertionError("failing_leaf_ref not found"));
                assertNotNull(leaf.getSubWorkflowId());
                failingLeafIdHolder[0] = leaf.getSubWorkflowId();
                assertFalse(workflowClient.getWorkflow(failingLeafIdHolder[0], true).getTasks().isEmpty());
            });
            ...
            Task leafInner = leaf.getTasks().stream()
            ...
            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                Workflow top = workflowClient.getWorkflow(topId, true);
                assertEquals(Workflow.WorkflowStatus.FAILED, top.getStatus(), "Top should be FAILED");
                assertEquals(Task.Status.CANCELED, statusOf(top, "top_cancelled_ref"),
                        "top_cancelled_ref should be CANCELED");
                assertNotNull(taskOf(top, "mid_ref").getReasonForIncompletion(),
                        "top.mid_ref should have reasonForIncompletion populated while FAILED");

                Workflow mid = workflowClient.getWorkflow(midId, true);
                assertEquals(Workflow.WorkflowStatus.FAILED, mid.getStatus(), "Mid should be FAILED");
                assertEquals(Task.Status.CANCELED, statusOf(mid, midCancelledSimpleRef),
                        "mid_cancelled_simple should be CANCELED");
                assertNotNull(taskOf(mid, "grandchild_ref").getReasonForIncompletion(),
                        "mid.grandchild_ref should have reasonForIncompletion populated while FAILED");

                Workflow grandchild = workflowClient.getWorkflow(grandchildId, true);
                assertEquals(Workflow.WorkflowStatus.FAILED, grandchild.getStatus(),
                        "Grandchild should be FAILED");
                assertEquals(Task.Status.CANCELED, statusOf(grandchild, grandchildCancelledSimpleRef),
                        "grandchild_cancelled_simple should be CANCELED");
                assertNotNull(taskOf(grandchild, "failing_leaf_ref").getReasonForIncompletion(),
            ...
            awaitWorkflowStatus(topId, Workflow.WorkflowStatus.RUNNING, 30, "Top must resume to RUNNING");
            awaitWorkflowStatus(midId, Workflow.WorkflowStatus.RUNNING, 30, "Mid must resume to RUNNING");
            awaitWorkflowStatus(grandchildId, Workflow.WorkflowStatus.RUNNING, 30, "Grandchild must resume to RUNNING");

            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                findActiveTask(grandchildId, grandchildCancelledSimpleRef,
                        "Expected an active grandchild_cancelled_simple after retry");
                assertNull(findActiveTask(grandchildId, "failing_leaf_ref",
                                "failing_leaf_ref must be active after retry").getReasonForIncompletion(),
                        "grandchild.failing_leaf_ref reasonForIncompletion must be cleared after retry walk-up");
            });

            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                findActiveTask(midId, midCancelledSimpleRef,
                        "Expected an active mid_cancelled_simple after retry");
                assertNull(findActiveTask(midId, "grandchild_ref",
                                "grandchild_ref must be active after retry").getReasonForIncompletion(),
                        "mid.grandchild_ref reasonForIncompletion must be cleared after retry walk-up");
            });

            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                Task topCancelledNow = findActiveTask(topId, "top_cancelled_ref",
                        "Expected an active top_cancelled_ref after retry");
                assertEquals(originalTopCancelledId, topCancelledNow.getSubWorkflowId(),
                        "Top level: subWorkflowId must be PRESERVED (in-place retry)");
                Workflow resumedChild = workflowClient.getWorkflow(topCancelledNow.getSubWorkflowId(), true);
                assertFalse(resumedChild.getStatus().isTerminal(),
            ...
            completeTask(findActiveTask(grandchildId, grandchildCancelledSimpleRef,
                    "grandchild_cancelled_simple active task missing"), TaskResult.Status.COMPLETED);
            completeTask(findActiveTask(midId, midCancelledSimpleRef,
                    "mid_cancelled_simple active task missing"), TaskResult.Status.COMPLETED);

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(originalTopCancelledId, topCancelledTaskRef,
                            "Inner task in resumed top-cancelled child must be active"));
            completeTask(findActiveTask(originalTopCancelledId, topCancelledTaskRef,
                    "top-cancelled inner task missing"), TaskResult.Status.COMPLETED);

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                    findActiveTask(failingLeafId, failingLeafTaskRef,
                            "Retried leaf task must be active"));
            completeTask(findActiveTask(failingLeafId, failingLeafTaskRef,
                    "leaf retried task missing"), TaskResult.Status.COMPLETED);

            awaitWorkflowStatus(topId, Workflow.WorkflowStatus.COMPLETED, 30,
                    "Top workflow should reach COMPLETED end-to-end");

        } finally {
            terminateAll(topWfName, midSubWfName, grandchildSubWfName, failingLeafWfName, topCancelledSubWfName);
        }
    }

    /**
     * The key correctness property of retry-in-place over fresh-spawn: tasks that
     * already COMPLETED inside the cancelled child must STAY COMPLETED after retry,
     * never re-execute.
     */
    @Test
    @DisplayName("Retry-in-place preserves COMPLETED tasks inside the cancelled child (no duplicate execution)")
    public void testRetryInPlacePreservesCompletedTasksInsideCancelledChild() {
        ...
        try {
            ...
            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                Task cancelledSub = findActiveTask(workflowId, cancelledSubRef, "cancelled sibling missing");
                assertNotNull(cancelledSub.getSubWorkflowId());
                findActiveTask(cancelledSub.getSubWorkflowId(), taskARef,
                        "taskA should be active in cancelled child");
            });

            String failingSubWorkflowId = findActiveTask(workflowId, failingSubRef, "missing").getSubWorkflowId();
            String originalCancelledChildId = findActiveTask(workflowId, cancelledSubRef, "missing").getSubWorkflowId();

            completeTask(findActiveTask(originalCancelledChildId, taskARef,
                    "taskA must be active in cancelled child"), TaskResult.Status.COMPLETED);

            await().atMost(10, TimeUnit.SECONDS).untilAsserted(() -> {
                Workflow ch = workflowClient.getWorkflow(originalCancelledChildId, true);
                assertEquals(Task.Status.COMPLETED, statusOf(ch, taskARef),
                        "taskA should be COMPLETED in cancelled child");
                findActiveTask(ch, taskBRef, "taskB should be active (taskA done, taskB scheduled/in-progress)");
            });
            Task originalTaskA = workflowClient.getWorkflow(originalCancelledChildId, true).getTasks().stream()
                    .filter(t -> taskARef.equals(t.getReferenceTaskName())
                            && t.getStatus() == Task.Status.COMPLETED)
                    .findFirst().orElseThrow();
            ...
            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                ...
                Workflow ch = workflowClient.getWorkflow(originalCancelledChildId, true);
                assertTrue(ch.getStatus().isTerminal(),
                        "Cancelled child should be terminal. Got: " + ch.getStatus());
                assertEquals(Task.Status.COMPLETED, statusOf(ch, taskARef),
            ...
            await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
                Task siblingTaskNow = findActiveTask(workflowId, cancelledSubRef,
                        "cancelled sibling task should be active after retry");
                assertEquals(originalCancelledChildId, siblingTaskNow.getSubWorkflowId(),
                        "subWorkflowId must be PRESERVED — retry-in-place");

                Workflow ch = workflowClient.getWorkflow(originalCancelledChildId, true);
                assertFalse(ch.getStatus().isTerminal(),
                        "Resumed child must be non-terminal. Got: " + ch.getStatus());

                assertNull(findActiveTask(workflowId, failingSubRef,
                                "failing_sub_ref must be active after retry").getReasonForIncompletion(),
                        "failing_sub_ref reasonForIncompletion must be cleared after retry walk-up");

                Task taskAAfterRetry = ch.getTasks().stream()
                        .filter(t -> taskARef.equals(t.getReferenceTaskName())
                                && t.getStatus() == Task.Status.COMPLETED)
                        .findFirst().orElseThrow(() -> new AssertionError(
                                "taskA must remain COMPLETED after retry — fresh-spawn would have re-executed it"));
                assertEquals(originalTaskAId, taskAAfterRetry.getTaskId(),
                        "taskA must be the same task instance — not a re-executed copy");
                assertEquals(originalTaskAEndTime, taskAAfterRetry.getEndTime(),
    ...
    /**
     * Retrying a sub-workflow (the child) re-runs its failed inner task in place
     * and the parent's SUB_WORKFLOW task is resumed with the same child id.
     */
    @Test
    ...
    public void testRetrySubWorkflowRetriesFailedInnerTask() {
        ...
        try {
            ...
            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                ...
                findActiveTask(sub.getSubWorkflowId(), innerTaskRef, "inner task should be active in child");
            });

            String childId = findActiveTask(workflowId, subRef, "sub task should be active").getSubWorkflowId();
            ...
            awaitWorkflowStatus(workflowId, Workflow.WorkflowStatus.FAILED, "Parent should be FAILED");
            awaitWorkflowStatus(childId, Workflow.WorkflowStatus.FAILED, "Child should be FAILED");
            assertNotNull(taskOf(workflowClient.getWorkflow(workflowId, true), subRef).getReasonForIncompletion(),
                    "parent sub_ref should have reasonForIncompletion populated while FAILED");

            workflowClient.retryWorkflow(List.of(childId));

            awaitWorkflowStatus(childId, Workflow.WorkflowStatus.RUNNING, "Child must be RUNNING after retry");
            awaitWorkflowStatus(workflowId, Workflow.WorkflowStatus.RUNNING, "Parent must be RUNNING after retry");

            await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
                Task retried = findActiveTask(childId, innerTaskRef,
                        "A fresh attempt of inner_task must be active after retry");
                assertNotEquals(failedTaskId, retried.getTaskId(),
                        "Retried task should be a new attempt (different taskId)");
                Task subNow = findActiveTask(workflowId, subRef, "parent sub_ref must be active");
                assertEquals(childId, subNow.getSubWorkflowId(),
                        "Parent's SUB_WORKFLOW task must still point at the same child");
                assertNull(subNow.getReasonForIncompletion(),
                        "parent sub_ref reasonForIncompletion must be cleared after retry walk-up");
            });

            completeTask(findActiveTask(childId, innerTaskRef, "retried task missing"),
                    TaskResult.Status.COMPLETED);

            awaitWorkflowStatus(workflowId, Workflow.WorkflowStatus.COMPLETED, 20, "Parent should reach COMPLETED");
    ...
    private Workflow completeTask(Task task, TaskResult.Status status) {
        return taskClient.updateTaskSync(task.getWorkflowInstanceId(), task.getReferenceTaskName(), status, task.getOutputData());
    }
    ...
    private void awaitWorkflowStatus(String workflowId, Workflow.WorkflowStatus expected, int seconds, String message) {
        await().atMost(seconds, TimeUnit.SECONDS).untilAsserted(() ->
                assertEquals(expected, workflowClient.getWorkflow(workflowId, false).getStatus(), message));
    }
}
