...
public class WorkflowRetryTests {
    ...
    public void testRetryWithSubWorkflow() {
        ...
        await().atMost(33, TimeUnit.SECONDS).untilAsserted(() -> {
            Workflow workflow1 = workflowClient.getWorkflow(workflowId, false);
            assertEquals(Workflow.WorkflowStatus.COMPLETED.name(), workflow1.getStatus().name(), "workflow " + workflowId + " did not complete");
        });
    ...
    public void testRetryWithSubWorkflowOptionalLevels() {
        ...
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            assertEquals(Workflow.WorkflowStatus.RUNNING.name(), child.getStatus().name());
    ...
    private void terminateExistingRunningWorkflows(String workflowName) {
        ...
        try {
            ...
            found.getResults().forEach(workflowSummary -> {
                    ...
                    workflowClient.terminateWorkflow(workflowSummary.getWorkflowId(), "terminate - retry test");
            });
        } catch(Exception e){
