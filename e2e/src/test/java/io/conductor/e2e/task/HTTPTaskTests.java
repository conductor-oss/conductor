...
public class HTTPTaskTests {
    ...
    public void HTTPAsyncCompleteTest() {
        ...
        await().pollInterval(5, TimeUnit.SECONDS).atMost(1, TimeUnit.MINUTES).untilAsserted(() -> {
            ...
            assertEquals(Task.Status.SCHEDULED, workflow.getTasks().getFirst().getStatus());
            assertNotNull(workflow.getTasks().getFirst().getOutputData().getOrDefault("response", null));
        });
        Workflow workflow = workflowClient.getWorkflow(workflowId, true);
        ...
        await().pollInterval(5, TimeUnit.SECONDS).atMost(1, TimeUnit.MINUTES).untilAsserted(() -> {
            Workflow workflowCompleted = workflowClient.getWorkflow(workflowId, false);
            assertEquals(Workflow.WorkflowStatus.COMPLETED, workflowCompleted.getStatus());
        });
        workflow = workflowClient.getWorkflow(workflowId, true);
        Map<String, Object> response = (Map<String, Object>) workflow.getTasks().getFirst().getOutputData().getOrDefault("response", Map.of());
