...
import com.netflix.conductor.redis.jedis.RedisSentinelJedisProvider;
import com.netflix.conductor.redis.jedis.TokenMapSupplierProvider;
...
public class RedisSentinelConfiguration {
    ...
    public JedisCommands jedisCommands(HostSupplier hostSupplier, RedisProperties properties) {
        return new RedisSentinelJedisProvider(hostSupplier, properties).get();
    }

    @Bean(name = READ_CLIENT_INJECTION_NAME)
    public JedisCommands readJedisCommands(HostSupplier hostSupplier, RedisProperties properties) {
        return new RedisSentinelJedisProvider(hostSupplier, properties).get();
    }
    ...
    public TokenMapSupplier tokenMapSupplier() {
        return new TokenMapSupplierProvider().get();
    }
