...
/** SQLite {@link SchemaDAO} — table {@code meta_schema_def}. */
public class SqliteSchemaDAO extends SqliteBaseDAO implements SchemaDAO {
    ...
    // The conditional insert leaves the decision to the primary key rather than to a preceding
    // read, and reports the loss as zero rows affected instead of an exception.
    //
    // DO NOTHING names the conflict it tolerates, unlike INSERT OR IGNORE, which swallows every
    // constraint violation and would let a NOT NULL failure be read as a lost race and retried.
