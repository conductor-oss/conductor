...
public class While extends Task<While> {
     ...
     * @param tasks
     */
    public While(String taskReferenceName, String condition, Task<?>... tasks) {
        super(taskReferenceName, TaskType.WHILE);
     ...
     * @param tasks
     */
    public While(String taskReferenceName, int loopCount, Task<?>... tasks) {
        super(taskReferenceName, TaskType.WHILE);
        Collections.addAll(this.loopTasks, tasks);
        this.loopCondition = getForLoopCondition(loopCount);
    }

    While(WorkflowTask workflowTask) {
        super(workflowTask);
    ...
    public While loopOver(Task<?>... tasks) {
        for (Task<?> task : tasks) {
