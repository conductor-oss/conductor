...
public class TargetWorkflowCollector implements WebhookConfig.WebhookConfigVisitor {
    ...
    @Getter private Map<String, Integer> workflowsToCompleteWebhooks;
    @Getter private Map<String, Object> workflowsToStart;

    public TargetWorkflowCollector(
            Map<String, Evaluator> evaluators, ParametersUtils parametersUtils) {
    ...
    public void visit(WebhookConfig webhookConfig) {
        if (StringUtils.isNotEmpty(webhookConfig.getExpression())) {
            ...
            Map<String, Integer> workflowsToComplete =
                    result.get("receiverWorkflowNamesToVersions").entrySet().stream()
                            .collect(
                                    Collectors.toMap(
                                            Map.Entry::getKey,
    ...
    private Map<String, Map<String, Object>> evaluateExpression(WebhookConfig webhookConfig) {
        String evaluatorType =
                webhookConfig.getEvaluatorType() != null
                        ? webhookConfig.getEvaluatorType()
                        : "javascript";
    ...
    /**
     * Method has side effect It removes the key from the map after retrieving it.
     *
     * <p>This is an improvement over the previous implementation where a getter had side effects
     */
