...
public class SubWorkflow extends WorkflowSystemTask {
	...
	public void start(Workflow workflow, Task task, WorkflowExecutor provider) throws Exception {
		...
		try {
			...
			String subWorkflowId = provider.startWorkflow(name, version, wfInput, correlationId, workflow.getWorkflowId(), task.getTaskId(), null, workflow.getTaskToDomain(), null);
