...
/**
 * An implementation of {@link TaskMapper} to map a {@link WorkflowTask} of type {@link TaskType#SWITCH} to a List
 * {@link Task} starting with Task of type {@link TaskType#SWITCH} which is marked as IN_PROGRESS, followed by
 * the list of {@link Task} based on the case expression evaluation in the Switch task.
 */
@Component
public class SwitchTaskMapper implements TaskMapper {
     ...
     * </li>
     * <li>
     * List of tasks based on the evaluation of {@link WorkflowTask#getEvaluatorType()} and
     * {@link WorkflowTask#getExpression()} are scheduled.
     * </li>
     * <li>
     * In the case of no matching {@link WorkflowTask#getEvaluatorType()}, workflow will be terminated with
     * error message.
     * In case of no matching result after the evaluation of the {@link WorkflowTask#getExpression()}, the
     * {@link WorkflowTask#getDefaultCase()} Tasks are scheduled.
     * </li>
     * </ul>
     */
    @Override
    public List<Task> getMappedTasks(TaskMapperContext taskMapperContext) {
        ...
        //if the tasks returned are empty based on evaluated result, then get the default case if there is one
        if (selectedTasks == null || selectedTasks.isEmpty()) {
            selectedTasks = taskToSchedule.getDefaultCase();
        }
        //once there are selected tasks that need to proceeded as part of the switch, get the next task to be
