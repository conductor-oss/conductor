...
public class ForkJoinDynamicTaskMapper implements TaskMapper {
    ...
    /**
     * This method gets the list of tasks that need to scheduled when the task to scheduled is of type {@link
     * TaskType#FORK_JOIN_DYNAMIC}. Creates a Fork Task, followed by the Dynamic tasks and a final JOIN task.
     * <p>The definitions of the dynamic forks that need to be scheduled are available in the {@link
     * WorkflowTask#getInputParameters()} which are accessed using the {@link TaskMapperContext#getTaskToSchedule()}.
     * The dynamic fork task definitions are referred by a key value either by {@link
     * WorkflowTask#getDynamicForkTasksParam()} or by {@link WorkflowTask#getDynamicForkJoinTasksParam()}
     * </p>
     ...
     *                                    <ul>
     *                                    <li>When the task after {@link TaskType#FORK_JOIN_DYNAMIC} is not a {@link TaskType#JOIN}</li>
     *                                    <li>When the input parameters for the dynamic tasks are not of type {@link Map}</li>
     *                                    </ul>
     */
    @Override
    ...
    /**
     * This method creates a FORK task and adds the list of dynamic fork tasks keyed by "forkedTaskDefs" and their names
     * keyed by "forkedTasks" into {@link Task#getInputData()}
     *
    ...
    Task createDynamicForkTask(WorkflowTask taskToSchedule, Workflow workflowInstance, String taskId,
        ...
        forkDynamicTask.getInputData()
            .put("forkedTaskDefs", dynForkTasks);    //TODO: Remove this parameter in the later releases
    ...
    /**
     * This method creates a JOIN task that is used in the {@link this#getMappedTasks(TaskMapperContext)} at the end to
     * add a join task to be scheduled after all the fork tasks
     *
     * @param workflowInstance: A instance of the {@link Workflow} which represents the workflow being executed.
    ...
    /**
     * This method is used to get the List of dynamic workflow tasks and their input based on the {@link
     * WorkflowTask#getDynamicForkTasksParam()}
     *
     * @param taskToSchedule:       The Task of type FORK_JOIN_DYNAMIC that needs to scheduled, which has the input
     *                              parameters
     * @param workflowInstance:     The instance of the {@link Workflow} which represents the workflow being executed.
     * @param dynamicForkTaskParam: The key representing the dynamic fork join json payload which is available in {@link
     *                              WorkflowTask#getInputParameters()}
     * @return a {@link Pair} representing the list of dynamic fork tasks in {@link Pair#getLeft()} and the input for
     * the dynamic fork tasks in {@link Pair#getRight()}
     * @throws TerminateWorkflowException : In case of input parameters of the dynamic fork tasks not represented as
     *                                    {@link Map}
     */
    @SuppressWarnings("unchecked")
    @VisibleForTesting
    Pair<List<WorkflowTask>, Map<String, Map<String, Object>>> getDynamicForkTasksAndInput(WorkflowTask taskToSchedule,
        Workflow workflowInstance,
        String dynamicForkTaskParam) throws TerminateWorkflowException {

        Map<String, Object> input = parametersUtils
            .getTaskInput(taskToSchedule.getInputParameters(), workflowInstance, null, null);
    ...
    /**
     * This method is used to get the List of dynamic workflow tasks and their input based on the {@link
     * WorkflowTask#getDynamicForkJoinTasksParam()}
     * <p><b>NOTE:</b> This method is kept for legacy reasons, new workflows should use the {@link
     * #getDynamicForkTasksAndInput}</p>
     *
     * @param taskToSchedule:   The Task of type FORK_JOIN_DYNAMIC that needs to scheduled, which has the input
     *                          parameters
     * @param workflowInstance: The instance of the {@link Workflow} which represents the workflow being executed.
     * @return {@link Pair} representing the list of dynamic fork tasks in {@link Pair#getLeft()} and the input for the
     * dynamic fork tasks in {@link Pair#getRight()}
     * @throws TerminateWorkflowException : In case of the {@link WorkflowTask#getInputParameters()} does not have a
     *                                    payload that contains the list of the dynamic tasks
     */
    @VisibleForTesting
    Pair<List<WorkflowTask>, Map<String, Map<String, Object>>> getDynamicForkJoinTasksAndInput(
        WorkflowTask taskToSchedule, Workflow workflowInstance) throws TerminateWorkflowException {
        ...
        DynamicForkJoinTaskList dynamicForkJoinTaskList = objectMapper
            .convertValue(paramValue, DynamicForkJoinTaskList.class);
        ...
        List<WorkflowTask> dynamicForkJoinWorkflowTasks = dynamicForkJoinTaskList.getDynamicTasks().stream()
            .peek(dynamicForkJoinTask -> dynamicForkJoinTasksInput.put(dynamicForkJoinTask.getReferenceName(),
                dynamicForkJoinTask.getInput())) //TODO create a custom pair collector
            .map(dynamicForkJoinTask -> {
                ...
                if (dynamicForkJoinWorkflowTask.getTaskDefinition() == null && StringUtils
                    .isNotBlank(dynamicForkJoinWorkflowTask.getName())) {
                    dynamicForkJoinWorkflowTask
                        .setTaskDefinition(metadataDAO.getTaskDef(dynamicForkJoinTask.getTaskName()));
                }
