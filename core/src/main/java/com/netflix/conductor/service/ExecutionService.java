...
public class ExecutionService {
    ...
    /**
     * Claims a batch of async system tasks from {@code queueName} for in-process execution: the
     * remote-worker poll contract applied to the system-task path. Each claimed task is persisted
     * as IN_PROGRESS and its queue message postponed by an execution lease <em>before</em> the task
     * is handed to the executor, so a message redelivered while the lease is live never duplicates
     * an in-flight blocking invocation. The message is deliberately not ACKed — lease expiry
     * doubles as crash recovery when the claiming node dies mid-invocation.
     *
     * <p>Differences from {@link #poll(String, String, String, int, int)}, each load-bearing for
     * the system-task path: operates on the caller's literal queue name (isolation-group queues),
     * performs no secrets substitution (the executor substitutes with a literal-input restore
     * before persisting), reads the worker-requested callbackAfterSeconds for the in-flight guard
     * instead of resetting it, and enforces no REST poll-timeout cap.
     *
     * @param queueName the queue to claim from, exactly as polled by SystemTaskWorker
     * @param count maximum number of tasks to claim
     * @param timeoutInMilliSeconds queue pop timeout
     * @return the claimed tasks, each carrying its pre-claim status for start/execute dispatch
     */
    public List<ClaimedSystemTask> claimSystemTasks(
            String queueName, int count, int timeoutInMilliSeconds) {
        List<String> taskIds = new LinkedList<>();
        List<ClaimedSystemTask> claimedTasks = new LinkedList<>();
        String taskType = QueueUtils.getTaskType(queueName);
        try {
            taskIds = queueDAO.pop(queueName, count, timeoutInMilliSeconds);
        } catch (Exception e) {
            LOGGER.error(
                    "Error polling for system tasks in queue: {}, count: {}", queueName, count, e);
            Monitors.error(this.getClass().getCanonicalName(), "claimSystemTasks");
            Monitors.recordTaskPollError(taskType, e.getClass().getSimpleName());
        }
        ...
        for (String taskId : taskIds) {
            try {
                ...
                // In-flight guard: an IN_PROGRESS task whose lease (or worker-requested callback)
                // has not elapsed is executing right now — this message is an early redelivery
                // (e.g. repaired by WorkflowRepairService on a queue impl that missed the
                // postponed message). Renew the remaining lease instead of claiming, so the
                // in-flight invocation is never duplicated and a due multi-turn callback is
                // never eaten.
                long now = System.currentTimeMillis();
                if (taskModel.getStatus() == TaskModel.Status.IN_PROGRESS
                        && taskModel.getCallbackAfterSeconds() > 0) {
                    long leaseExpiryMillis =
                            taskModel.getUpdateTime() + taskModel.getCallbackAfterSeconds() * 1000L;
                    if (leaseExpiryMillis > now) {
                        long remainingSeconds = Math.max(1, (leaseExpiryMillis - now) / 1000);
                        queueDAO.postpone(
                                queueName,
                                taskId,
                                taskModel.getWorkflowPriority(),
                                remainingSeconds);
                        LOGGER.debug(
                                "Task: {} holds a live execution lease; postponed redelivered"
                                        + " message in queue: {} by {} seconds",
                                taskId,
                                queueName,
                                remainingSeconds);
                        continue;
                ...
                TaskModel.Status preClaimStatus = taskModel.getStatus();
                long leaseSeconds =
                        (taskDef != null && taskDef.getResponseTimeoutSeconds() > 0)
                                ? taskDef.getResponseTimeoutSeconds()
                                : properties.getSystemTaskClaimLeaseDuration().getSeconds();
                taskModel.setStatus(TaskModel.Status.IN_PROGRESS);
                if (taskModel.getStartTime() == 0) {
                    taskModel.setStartTime(now);
                    Monitors.recordQueueWaitTime(
                            taskModel.getTaskDefName(), taskModel.getQueueWaitTime());
                }
                taskModel.setCallbackAfterSeconds(leaseSeconds);
                taskModel.setWorkerId(Utils.getServerId());
                taskModel.incrementPollCount();
                executionDAOFacade.updateTask(taskModel);
                queueDAO.postpone(queueName, taskId, taskModel.getWorkflowPriority(), leaseSeconds);
                adjustDeciderQueuePostpone(taskModel, taskDef);
                try {
                ...
                claimedTasks.add(new ClaimedSystemTask(taskModel, preClaimStatus));
            } catch (Exception e) {
                // db operation failed for dequeued message, re-enqueue with a delay
                LOGGER.warn(
                        "DB operation failed while claiming task: {}, postponing message in queue",
                        taskId,
                        e);
                Monitors.recordTaskPollError(taskType, e.getClass().getSimpleName());
                queueDAO.postpone(queueName, taskId, 0, queueTaskMessagePostponeSecs);
