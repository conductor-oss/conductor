...
public class TestAnnotatedWorkflowSystemTask {
    ...
    // ── Issue #1321: declare a queue-message lease over blocking method invocations ──
    //
    ...
    // method again. The adapter declares an execution lease of the task's
    // responseTimeoutSeconds; AsyncSystemTaskExecutor extends the message's
    // visibility by that amount before invoking the method.

    static class BlockingCallBean {
    ...
    @Test
    public void testExecutionLeaseIsResponseTimeoutFromTaskDef() throws Exception {
        Method method = BlockingCallBean.class.getMethod("blockingCall");
        AnnotatedWorkflowSystemTask systemTask =
                new AnnotatedWorkflowSystemTask(
                        "llm_like_task",
                        method,
                        new BlockingCallBean(),
                        createAnnotation("llm_like_task"));
        ...
        assertEquals(Optional.of(Duration.ofSeconds(3600)), systemTask.getExecutionLease(task));
    }

    @Test
    public void testExecutionLeaseEmptyWithoutTaskDef() throws Exception {
        Method method = BlockingCallBean.class.getMethod("blockingCall");
        AnnotatedWorkflowSystemTask systemTask =
                new AnnotatedWorkflowSystemTask(
                        "no_taskdef_task",
                        method,
                        new BlockingCallBean(),
                        createAnnotation("no_taskdef_task"));

        TaskModel task = createTask(Map.of());
        task.setTaskType("no_taskdef_task");

        assertEquals(Optional.empty(), systemTask.getExecutionLease(task));
    }

    @Test
    public void testExecutionLeaseEmptyWithZeroResponseTimeout() throws Exception {
        Method method = BlockingCallBean.class.getMethod("blockingCall");
        AnnotatedWorkflowSystemTask systemTask =
                new AnnotatedWorkflowSystemTask(
                        "zero_timeout_task",
                        method,
                        new BlockingCallBean(),
                        createAnnotation("zero_timeout_task"));
