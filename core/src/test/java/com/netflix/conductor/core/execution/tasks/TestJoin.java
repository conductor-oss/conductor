...
public class TestJoin {
    ...
    @Test
    public void testResolveJoinOnReferences_NonSwitchTask() {
        ...
        var join = new Join(properties, systemTaskRegistry);
        List<String> resolved = join.resolveJoinOnReferences(workflow, List.of("task1"));

        assertEquals(1, resolved.size());
        assertEquals("task1", resolved.get(0));
    }

    @Test
    public void testResolveJoinOnReferences_SwitchTaskNotCompleted() {
        ...
        var join = new Join(properties, systemTaskRegistry);
        List<String> resolved = join.resolveJoinOnReferences(workflow, List.of("switchTask"));

        assertEquals(1, resolved.size());
        assertEquals("switchTask", resolved.get(0));
    }

    @Test
    public void testResolveJoinOnReferences_SwitchTaskWithChildTasks() {
        ...
        assertEquals(1, resolved.size());
        assertEquals("t2", resolved.get(0)); // Should resolve to the last task in the case branch
    }

    @Test
    public void testResolveJoinOnReferences_SwitchTaskWithDefaultCase() {
        ...
        assertEquals(1, resolved.size());
        assertEquals("defaultRef", resolved.get(0));
    }

    @Test
    public void testResolveJoinOnReferences_SwitchTaskWithEmptyBranch() {
        ...
        assertEquals(1, resolved.size());
        assertEquals("switchTask", resolved.get(0)); // Falls back to switch itself
    }

    @Test
    public void testResolveJoinOnReferences_TaskNotScheduled() {
        // Reference to a task that is not yet scheduled should keep original reference
        WorkflowModel workflow = new WorkflowModel();
        // No tasks added to workflow

        var join = new Join(properties, systemTaskRegistry);
        List<String> resolved = join.resolveJoinOnReferences(workflow, List.of("notScheduledTask"));

        assertEquals(1, resolved.size());
        assertEquals("notScheduledTask", resolved.get(0));
    }

    @Test
    public void testResolveJoinOnReferences_MultipleTasksWithSwitch() {
        ...
        List<String> resolved =
                join.resolveJoinOnReferences(workflow, List.of("regularTask", "switchTask"));

        assertEquals(2, resolved.size());
        assertEquals("regularTask", resolved.get(0));
        assertEquals("switchChild", resolved.get(1)); // Switch resolved to child
    }
    ...
    public void testNestedSwitch_TerminalTaskResolution() {
        ...
        WorkflowTask innerChildTask =
                createWorkflowTaskDef("innerChild", "innerChildRef", "SIMPLE");

        Map<String, List<WorkflowTask>> innerDecisionCases = new HashMap<>();
        innerDecisionCases.put("innerCase", List.of(innerChildTask));

        WorkflowTask innerSwitchTaskDef =
                createWorkflowTaskDef("innerSwitch", "innerSwitchRef", "SWITCH");
        ...
        var innerChildTaskModel =
                createTask("innerChildRef", TaskModel.Status.COMPLETED, false, false);
        ...
        String lastTaskRef =
                switchSystemTask.getTerminalTaskRef(workflow, outerSwitchTask, systemTaskRegistry);
