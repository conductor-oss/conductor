...
public class JoinTest {
    ...
    public void testAgentJoinUsesWorkflowTaskMarkerForIterationQualifiedDynamicTool() {
        ...
        Map<String, Object> httpOutput =
                Map.of("response", Map.of("statusCode", 200, "body", Map.of("temperature", 64)));
    ...
    public void testAgentJoinPrefersRuntimeToolMarkerOverWorkflowTaskMarker() {
        ...
        assertTrue(join.execute(workflow, joinTask, null));
        Map<String, Object> compact = (Map<String, Object>) joinTask.getOutputData().get("weather");
        assertEquals("runtime-weather", compact.get("_agent_tool_name"));
        assertEquals(toolOutput, compact.get("_agent_tool_output"));
    }

    @Test
    public void testAgentJoinPrefersNestedRuntimeMarkerOverWorkflowTaskMarker() {
        Join join = new Join(mock(ConductorProperties.class));

        Map<String, Object> toolOutput = Map.of("result", "delegated");
        TaskModel forkedTask = forkedTaskWithOutput("delegate", toolOutput);
        forkedTask.setInputData(
                Map.of("workflowInput", Map.of("_agent_tool_name", "runtime-delegate")));
        forkedTask
                .getWorkflowTask()
                .setInputParameters(Map.of("_agent_tool_name", "definition-delegate"));
        ...
        TaskModel joinTask = joinTaskOn("delegate");

        assertTrue(join.execute(workflow, joinTask, null));
        assertEquals(
                Map.of("_agent_tool_name", "runtime-delegate", "_agent_tool_output", toolOutput),
                joinTask.getOutputData().get("delegate"));
    }
