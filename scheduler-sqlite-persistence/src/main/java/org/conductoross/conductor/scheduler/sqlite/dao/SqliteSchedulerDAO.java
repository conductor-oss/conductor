 ...
 * db/migration_scheduler_sqlite}). Uses SQLite-compatible upsert syntax ({@code INSERT OR REPLACE})
 * and {@code json_extract()} for JSON field ordering. Designed for single-node / embedded
 * deployments where a full SQL server is not available.
 */
public class SqliteSchedulerDAO implements SchedulerDAO {
    ...
    public void updateSchedule(WorkflowSchedule schedule) {
        // INSERT OR REPLACE deletes the old row (if any) and inserts a fresh one.
        // This is SQLite's upsert idiom and is safe because scheduler_name is the PRIMARY KEY.
        String sql =
    ...
    public List<WorkflowScheduleExecution> getExecutionRecords(String scheduleName, int limit) {
        String sql =
                "SELECT json_data FROM scheduler_execution "
                        + "WHERE schedule_name = ? "
                        + "ORDER BY CAST(json_extract(json_data, '$.executionTime') AS INTEGER) DESC "
                        + "LIMIT ?";
