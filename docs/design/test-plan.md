...
The conductor branch is a **poll-only, in-process** integration through core's
`WorkflowExecutor`. It has: no socket, no wire format, no streaming, no push callbacks,
...
|---|---|---|---|
| `EmbeddedA2AAgent` | Real test double (embedded HTTP agent) | `FakeWorkflowExecutor` — a real, in-process core executor implementation | **Mirror** |
...
| `A2ASdkInteropTest` | Interop with a non-Conductor A2A SDK agent | No wire protocol; execution is through the core executor | **N/A — no wire format** |
...
Net: the four "Mirror" rows (`FakeWorkflowExecutor` + three e2e classes) plus the extended
...
|---|---|---|
| `FakeWorkflowExecutor.java` | Scriptable core executor implementation | testing.md §2 |
...
- **Cancel:** `cancel(...)` propagates the reason to the fake and sets task `CANCELED`; a
  best-effort cancel still marks the task `CANCELED` even if the executor throws.
...
- **Live deployment:** an opt-in integration test requires deployed agent metadata and a running
  Conductor server. It belongs in an integration-test module rather than the `:ai` unit suite.
...
The whole suite runs in-process against `FakeWorkflowExecutor`; no socket, container, or
...
- No mock frameworks; `FakeWorkflowExecutor` implements the real core interface (AGENTS.md).
