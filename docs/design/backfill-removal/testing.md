...
1. **Production code compiles without the feature.**
   ...
   `reindexAgentExecutions`, `AGENT_CLASSIFIER_BACKFILL_VERSION`, or
   `AGENT_CLASSIFIER_BACKFILL_VERSION_VALUE`.
...
4. **No regression elsewhere.**
   ...
   Full suite passes. In particular, other `AgentSpanDeploymentContractEndToEndTest`
...
integration tests: any `@SpringBootTest`-based agentspan test that boots the
context (including `AgentSpanDeploymentContractEndToEndTest` itself) will fail
