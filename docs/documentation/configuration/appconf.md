...
| systemTaskWorkerThreadCount                 | int      | The number of threads in the shared threadpool for system task workers. It is also the maximum number of tasks in flight across all task types on that pool (types with a `taskWorkerConfigs` dedicated pool or an isolation group are not counted). Example: `8` if there are 4 processors (2x4)                                                                   | Default is 2 times the number of available processors   |
...
| taskWorkerConfigs                           | Map      | Per-task-type worker overrides, keyed by task type. `<TYPE>.threadCount` > 0 gives the type its own dedicated thread pool of that size, which is also its in-flight cap — at most `threadCount` tasks of the type run concurrently. Types without an entry share the common pool. Example: `taskWorkerConfigs.HTTP.threadCount=20` | Default is empty (all types share the common pool)      |
