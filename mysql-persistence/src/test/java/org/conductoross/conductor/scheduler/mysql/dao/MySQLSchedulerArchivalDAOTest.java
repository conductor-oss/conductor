...
/**
 * Runs the full {@link AbstractSchedulerArchivalDAOTest} contract suite against a MySQL database
...
@TestPropertySource(
        properties = {
            "spring.datasource.url=jdbc:tc:mysql:8.0:///scheduler_archival_test",
            "spring.datasource.username=test",
            "spring.datasource.password=test",
            "spring.datasource.hikari.maximum-pool-size=4",
            "spring.flyway.enabled=false"
        })
public class MySQLSchedulerArchivalDAOTest extends AbstractSchedulerArchivalDAOTest {
    ...
    public void cleanUp() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            // module test datasource sets hikari auto-commit=false — delete must commit
