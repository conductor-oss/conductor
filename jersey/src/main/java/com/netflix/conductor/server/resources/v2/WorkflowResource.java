 ...
 *
 */
@Api(value="/v2/workflow", produces=MediaType.APPLICATION_JSON, consumes=MediaType.APPLICATION_JSON, tags="Workflow Management")
@Path("/v2/workflow")
...
public class WorkflowResource {
    ...
    @Inject
    public WorkflowResource(WorkflowExecutor workflowExecutor, ExecutionService executionService, MetadataService metadataService, Configuration config) {
    ...
    public String startWorkflow (StartWorkflowRequest request) throws Exception {
        WorkflowDef def = metadataService.getWorkflowDef(request.getName(), request.getVersion());
        if(def == null){
            throw new ApplicationException(Code.NOT_FOUND, "No such workflow found by name=" + request.getName() + ", version=" + request.getVersion());
        }
        return workflowExecutor.startWorkflow(def.getName(), def.getVersion(), request.getCorrelationId(), request.getInput(), null, request.getTaskToDomain());
    }
    ...
    public String startWorkflow (
        ...
        return workflowExecutor.startWorkflow(def.getName(), def.getVersion(), correlationId, input, null);
    }
    ...
    public Map<String, List<Workflow>> getWorkflows(@PathParam("name") String name,
            ...
            for(String correlationId : correlationIds) {
                List<Workflow> ws = executionService.getWorkflowInstances(name, correlationId, includeClosed, includeTasks);
    ...
    public Workflow getExecutionStatus(
        ...
        return executionService.getExecutionStatus(workflowId, includeTasks);
    }
    ...
    public SearchResult<WorkflowSummary> search(
        ...
        return executionService.search(query, freeText, start, size, convert(sort));
    }
    ...
    public SearchResult<WorkflowSummary> searchWorkflowsByTasks(
        ...
        return executionService.searchWorkflowByTasks(query, freeText, start, size, convert(sort));
    }
