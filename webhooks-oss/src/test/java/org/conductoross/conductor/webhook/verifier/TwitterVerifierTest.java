...
public class TwitterVerifierTest {
    ...
    public void testProperTwitterVerifier() throws NoSuchAlgorithmException, InvalidKeyException {
        ...
        Assertions.assertTrue(
                twitterVerifier.verify(webhookConfig, incomingWebhookEvent).isEmpty());
    }
    ...
    public void testNotProperTwitterVerifier()
        ...
        httpHeaders.put(
                webhookConfig.getHeaders().keySet().iterator().next(),
                List.of("sha256=" + HashUtils.computeBase64HmacSha256("testing", body)));
        incomingWebhookEvent.setHeaders(httpHeaders);
        incomingWebhookEvent.setBody(body);
    ...
    public void testHandlePingEvent() throws NoSuchAlgorithmException, InvalidKeyException {
        WebhookConfig webhookConfig = createWebhook();
        String body = "information";
        ...
        String actual =
                "{\"response_token\":\"sha256="
                        + HashUtils.computeBase64HmacSha256(
                                webhookConfig.getSecretValue(), map.get("crc_token").toString())
                        + "\"}";
