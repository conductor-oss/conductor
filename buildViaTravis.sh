...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
  ...
  case "$TRAVIS_TAG" in
  *-alpha\.* | *-beta\.*)
    ./gradlew -Prelease.travisci=true -PbintrayUser="${bintrayUser}" -PbintrayKey="${bintrayKey}" -PsonatypeUsername="${sonatypeUsername}" -PsonatypePassword="${sonatypePassword}" snapshot coveralls --info --stacktrace
      ;;
  *-rc\.*)
